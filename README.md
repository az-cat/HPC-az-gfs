# Azure HPC Cluster with GFS attached
...
3. Clone the repository, `git clone https://github.com/az-cat/HPC-az-gfs`
4. Update the Batch Shipyard RemoteFS credentials file with Service Principal Auth, [required entries](https://github.com/az-cat/HPC-az-gfs/blob/master/credentials.yaml)
...
![alt text](https://github.com/az-cat/HPC-az-gfs/blob/master/support/azhpc_gfs_arch.png)
...
- NFS shared from the jumpbox and located at `/mnt/scratch`, created in the hn-setup script here: [hn-setup_gfs.sh](https://github.com/az-cat/HPC-az-gfs/blob/master/script/hn-setup_gfs.sh#L58-L72)
- GFS shared from the storage cluster mounted at `/mnt/gfs`, created using Batch Shipyard, [link](http://batch-shipyard.readthedocs.io/en/latest/65-batch-shipyard-remote-fs/), here in [create_cluster.sh](https://github.com/az-cat/HPC-az-gfs/blob/master/create_cluster.sh#L34-L39)
- Three 5TB Azure Files shares mounted to the jumpbox at `/mnt/lts1`,`/mnt/lts2`,`/mnt/lts3`. This is a CIFS share and can be mounted to both the Windows and Linux operating systems. These Azure File shares are subject to performance limits specified [here](https://docs.microsoft.com/en-us/azure/azure-subscription-service-limits#azure-files-limits). The size can be altered by increasing the quota here: [create_cluster.sh](https://github.com/az-cat/HPC-az-gfs/blob/master/create_cluster.sh#L66)
...
![alt text](https://github.com/az-cat/HPC-az-gfs/blob/master/support/workload_storage_movement.png)
...
After the file server is created the compute cluster is created in a seperate subnet. Currently an independant azuredeploy.json is used in this repository, the application configuration scripts that are used are the ones found in the AHOD-HPC repository. For this deployment a [Virtual Machine Scale Set is created](https://github.com/az-cat/HPC-az-gfs/blob/master/azuredeploy.json#L298-L350), [a jumpbox is created](https://github.com/az-cat/HPC-az-gfs/blob/master/azuredeploy.json#L231-L260) and an [extension is run on the jumpbox](https://github.com/az-cat/HPC-az-gfs/blob/master/azuredeploy.json#L261-L288). The extension downloads and calls the [hn-setup_gfs.sh script](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/hn-setup_gfs.sh) which configures the headnode, installs and starts the NFS server, and then launches the [cn-setup_gfs.sh script](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/hn-setup_gfs.sh) which mounts the GFS and NFS file server on all of the compute nodes.
...
![alt text](https://github.com/az-cat/HPC-az-gfs/blob/master/support/credentials.png) 
...
### Scripts
  ...
  The `azuredeploy.json` ARM template calls the two scripts that are located in the `scripts` directory. These scripts are used for the configuration of the head node [hn-setup_gfs.sh](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/hn-setup_gfs.sh) and the compute nodes [cn-setup_gfs.sh](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/cn-setup_gfs.sh). They are designed to be used when there is a Gluster File Server inside of the VNET that the template is deployed in.

  `hn-setup_gfs.sh` performs a number of basic node configuration commands. Installing needed packages and starting the NFS server. [Passwordless authentication](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/hn-setup_gfs.sh#L99-L109) is setup in such a way to allow seperate home directory on each of the nodes,this protects connectivity from being lost in the event that the NFS server locks up. An additional reason that this step was taken is because the jumpbox is not necessarily the same VM type as the compute nodes. The [host list is populated through an nmap](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/hn-setup_gfs.sh#L99-L109) instead of relying on a response from the Azure CLI. [Ganglia is installed](https://github.com/az-cat/HPC-az-gfs/blob/master/scripts/hn-setup_gfs.sh#L55-L57) on the jumpbox and all the compute nodes as well. Finally the script installs the selected application on the share, this installation is actually done from the first node in the hostlist.
...
### ARM Template
  - `azuredeploy.json` is the primary deployment script. It creates all of the resources needed for this example. This template does not affect the file server at all, that deployment is completely managed by Batch Shipyard.
